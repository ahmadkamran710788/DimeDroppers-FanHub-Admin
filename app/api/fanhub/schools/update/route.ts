...
/**
 * Proxy for FanHub "Update School" (Setup Wizard — Step 1 re-submit).
 *
 * The browser PUTs multipart/form-data here with `?schoolId=…`; this server-only handler
 * injects the secret `x-fanhub-key` header and forwards the body to
 * `/fanhub/schools/{schoolId}`. The key (config.fanhubApiKey) is never exposed to the
 * client bundle. `schoolId` rides in the query string so the internal route can stay static.
 */
export async function PUT(request: Request) {
  ...
  try {
    ...
    const upstream = await fetch(`${config.apiUrl}/${routes.api.updateSchool(schoolId)}`, {
      method: "PUT",
