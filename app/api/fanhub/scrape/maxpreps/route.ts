...
/**
 * Proxy for FanHub "Scrape MaxPreps schedule" (Setup Wizard — Step 2, section A).
 *
 * The browser posts multipart/form-data (the schedule image/PDF in a `file` field) here
 * with `?schoolId=…`; this server-only handler injects the secret `x-fanhub-key` header
 * and forwards the body to `/fanhub/scrape/maxpreps/{schoolId}`. The key
 * (config.fanhubApiKey) is never exposed to the client bundle. `schoolId` rides in the
 * query string so the internal route can stay static.
 */
export async function POST(request: Request) {
  ...
  try {
    ...
    const upstream = await fetch(`${config.apiUrl}/${routes.api.scrapeMaxpreps(schoolId)}`, {
      method: "POST",
